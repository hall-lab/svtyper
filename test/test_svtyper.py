...
class TestCigarParsing(unittest.TestCase):
    def test_cigarstring_to_tuple(self):
        ...
        self.assertEqual(svtyper.cigarstring_to_tuple(string1),
                [(5, 5), (4, 3), (2, 2), (3, 1),
                    (0, 5), (1, 3), (6, 2), (8, 2),
                    (7, 1)])
    ...
    def test_get_query_pos_from_cigar(self):
        ...
        cigar = svtyper.cigarstring_to_tuple(cigar_string)
        query_pos = svtyper.get_query_pos_from_cigar(cigar, True)
        ...
        query_pos = svtyper.get_query_pos_from_cigar(cigar, False)
    ...
    def test_get_reference_end_from_cigar(self):
        ...
        cigar = svtyper.cigarstring_to_tuple(cigar_string)
        self.assertEqual(svtyper.get_reference_end_from_cigar(1, cigar), 11)

    def test_get_start_diagonal(self):
        cigar_string = '2S5M3D1I1M3S'
        split_piece = svtyper.SplitRead.SplitPiece(1, 25, True, svtyper.cigarstring_to_tuple(cigar_string), 60)
        self.assertEqual(svtyper.get_start_diagonal(split_piece), 23)
        split_piece2 = svtyper.SplitRead.SplitPiece(1, 25, False, svtyper.cigarstring_to_tuple(cigar_string), 60)
        self.assertEqual(svtyper.get_start_diagonal(split_piece2), 23)

    def test_get_end_diagonal(self):
        cigar_string = '2S5M3D2I1M3S'
        split_piece = svtyper.SplitRead.SplitPiece(1, 25, True, svtyper.cigarstring_to_tuple(cigar_string), 60)
        split_piece.set_reference_end(34)
        self.assertEqual(svtyper.get_end_diagonal(split_piece), 34 - (2 + 8))
        split_piece2 = svtyper.SplitRead.SplitPiece(1, 25, False, svtyper.cigarstring_to_tuple(cigar_string), 60)
        split_piece2.set_reference_end(34)
        self.assertEqual(svtyper.get_end_diagonal(split_piece2), 34 - (2 + 8))
